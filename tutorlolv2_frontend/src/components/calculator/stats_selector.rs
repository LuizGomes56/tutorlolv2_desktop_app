use crate::{components::calculator::CurrentPlayerVolatileAttrs, url, utils::to_pascal_case};
use yew::{
...
pub struct StatsSelectorProps {
    pub volatile_attrs_handle: UseStateHandle<CurrentPlayerVolatileAttrs>,
}

#[function_component(StatsSelector)]
pub fn stats_selector(props: &StatsSelectorProps) -> Html {
    let stats = props.volatile_attrs_handle.champion_stats;
            ...
            html! {
                <>
                    ...
                    <input
                        ...
                        oninput={{
                            let volatile_attrs_handle = props.volatile_attrs_handle.clone();
                            Callback::from(move |e: InputEvent| {
                                let target = e.target_unchecked_into::<web_sys::HtmlInputElement>();
                                let value = target.value().parse::<f64>().unwrap_or(0.0);
                                let mut volatile_attrs = *volatile_attrs_handle;
                                let mut attrs = stats;
                                attrs.$field = value;
                                volatile_attrs.champion_stats = attrs;
                                volatile_attrs_handle.set(volatile_attrs);
                            })
                        }}
                    />
                </>
